...
void *drukowanie(void* par)
{
    ...
    while(1)
    {        
        if((((struct send*)par)->clear)==1)
        {
            (((struct send*)par)->clear)=0;
            for(int i=0;i<27;i++)
            {
                for(int j=0;j<53;j++)
                {
...
void move_up(struct send* zmienna, int *last)
{
    zmienna->ruch='A';
    *last=0;
}
void move_down(struct send* zmienna, int *last)
{
    zmienna->ruch='B';
    *last=1;
}
void move_right(struct send* zmienna, int *last)
{
    zmienna->ruch='C';
    *last=2;
}
void move_left(struct send* zmienna, int *last)
{
    zmienna->ruch='D';
    *last=3;
}
...
int main ()
{
    ...
    pthread_t wyjscie;
    pthread_create(&wyjscie,NULL,exito,NULL);
    sleep(1);
    int last=0;
    
    //poddalem sie z algorytmem przechodzenia,nie udalo mi sie napisac na tyle dobrego
    //by bot stal sie nagle "inteligentny"
	...
	while(1)
    {
        ...
        {
                int num=rand()%4;
                if(num==0 && zmienna->tablica[1][2]=='%') continue;
                else if(num==1 && zmienna->tablica[3][2]=='%') continue;
                else if(num==2 && zmienna->tablica[2][3]=='%') continue;
                else if(num==3 && zmienna->tablica[2][1]=='%') continue;
                    
                if(num==0) move_up(zmienna,&last);
                else if(num==1) move_down(zmienna,&last);
                else if(num==2) move_right(zmienna,&last);
                else if(num==3) move_left(zmienna,&last); 
                break;
        }
        refresh();
        sem_post(sem2);
